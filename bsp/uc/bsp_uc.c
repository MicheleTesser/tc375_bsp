 ...
 *
 *    V2  01.2021  RO
 *    - Update of intrinsics functions redesigned in the latest TRILLVM toolchain
 *
 **************************************************************************************************
...
void bsp_uc_port_EnableInput(uint32_t port, uint32_t pinIndex, UC_PORT_INPUT_MODE_e mode)
{
	...
	/* to select which byte field in the register to work on */
	bitOffset = 8 * (pinIndex % 4);
	bitMask = (uint32_t) 0xFF << bitOffset;

	/* Set PCx field correspondingly for Input */
	UC_PORT_IOCR_REG(port, pinIndex) = (UC_PORT_IOCR_REG(port, pinIndex) & ~(bitMask)) | ((mode << 3) << bitOffset);
}
...
void bsp_uc_port_EnableOutput(uint32_t port, uint32_t pinIndex, uint32_t source, UC_PORT_PL_LEVEL_e pl, UC_PORT_PD_SPEED_e pd)
{
	...
	bitOffset = 8 * (pinIndex % 4);
	bitMask = (uint32_t) 0xFF << bitOffset;
	UC_PORT_IOCR_REG(port, pinIndex) = (UC_PORT_IOCR_REG(port, pinIndex) & ~(bitMask)) | (((0x10 | source) << 3) << bitOffset);

    /* Set Pin output strength and levels in PDR register */
    unlock_wdtcon();
    bitOffset = 4 * (pinIndex % 8);
	bitMask = (uint32_t) 0xF << bitOffset;
	UC_PORT_PDR_REG(port, pinIndex) = (UC_PORT_PDR_REG(port, pinIndex) & ~(bitMask)) | (((pl << 3) | pd) << bitOffset);
    lock_wdtcon();
}
...
uint32_t bsp_uc_core_GetCurrentCore(void)
{
	...
	uint32_t coreId = __builtin_tricore_mfcr(CPU_CORE_ID);
